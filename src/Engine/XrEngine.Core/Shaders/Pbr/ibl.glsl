...
{
    vec4 textureSample = texture(uLambertianEnvSampler, uIBL.EnvRotation * n);
    textureSample.rgb *= uIBL.EnvIntensity;
...
{
    vec4 textureSample = textureLod(uGGXEnvSampler, uIBL.EnvRotation * reflection, lod);
    textureSample.rgb *= uIBL.EnvIntensity;
...
{
    vec4 textureSample =  textureLod(uCharlieEnvSampler, uIBL.EnvRotation * reflection, lod);
    textureSample.rgb *= uIBL.EnvIntensity;
    return textureSample;
}
...
vec3 getIBLGGXFresnel(vec3 n, vec3 v, float roughness, vec3 F0, float specularWeight)
{
    ...
    vec2 f_ab = texture(uGGXLUT, brdfSamplePoint).rg;
...
{
    float NdotV = clampedDot(n, v);
    float lod = roughness * float(uIBL.MipCount - 1);
...
{
    float framebufferLod = log2(float(uTransmissionFramebufferSize.x)) * applyIorToRoughness(roughness, ior);
    vec3 transmittedLight = textureLod(uTransmissionFramebufferSampler, fragCoord.xy, framebufferLod).rgb;
    ...
    vec3 position, mat4 modelMatrix, mat4 viewMatrix, mat4 projMatrix, float ior, float thickness, vec3 attenuationColor, float attenuationDistance, float dispersion)
{
    ...
    vec2 brdf = texture(uGGXLUT, brdfSamplePoint).rg;
...
{
    float NdotV = clampedDot(n, v);
    float lod = sheenRoughness * float(uIBL.MipCount - 1);
    vec3 reflection = normalize(reflect(-v, n));

    vec2 brdfSamplePoint = clamp(vec2(NdotV, sheenRoughness), vec2(0.0, 0.0), vec2(1.0, 1.0));
    float brdf = texture(uCharlieLUT, brdfSamplePoint).b;
