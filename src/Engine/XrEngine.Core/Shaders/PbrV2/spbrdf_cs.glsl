...
void main(void)
{
	...
	float cosLo = float(gl_GlobalInvocationID.x) / float(imageSize(LUT).x);
	float roughness = float(gl_GlobalInvocationID.y) / float(imageSize(LUT).y);
	...
	imageStore(LUT, ivec2(gl_GlobalInvocationID), vec4(DFG1, DFG2, 0.0, 0.0) * InvNumSamples);
}
