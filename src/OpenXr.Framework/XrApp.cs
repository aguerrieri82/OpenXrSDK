...
using OpenXr.Framework.Entities;
    ...
    public unsafe class XrApp : IDisposable, IXrSession
    {
        ...
        public void RenderFrame()
        {
            ...
            if (state.ShouldRender != 0)
            {
                ...
                if (isPosValid)
                    layers = _layers.Render(_views, _swapchains);

            }

            EndFrame(state.PredictedDisplayPeriod, ref layers);
        }


        protected void EndFrame(long displayTime, ref CompositionLayerBaseHeader*[]? layers)
        {
            fixed (CompositionLayerBaseHeader** pLayers = layers)
            {
                var frameEndInfo = new FrameEndInfo()
                {
                    ...
                    LayerCount = layers == null ? 0 : (uint)layers.Length,
                    Layers = pLayers
                };
